...
class ProduserController {
   ...
   getAllProduser = async (req, res, next) => {
      try {
         ...
         const produserCount = await Produser.countDocuments();
         const apiFeaturesFilter = new ApiFeatures(Produser.find(), req.query)
            .searchByName()
            .filter();

         let produsers = await apiFeaturesFilter.query;
         let filterCountProdusers = produsers.length;
         ...
         produsers = await apiFeaturesFilterPaginagtion.query;

         res.json({
            success: true,
            produsers,
            produserCount,
            filterCountProdusers,
         });
      } catch (e) {
   ...
   getAllProdusersAdmin = async (req, res, next) => {
