...
function ProductsRecommendation() {
  ...
  const { keyword, category, price, ratings, currentPage, queryFetch } =
  ...
  return (
    <>
      ...
      <h2 className="productsHeading">Recommendation</h2>
      <form
        onSubmit={(e) => {
          e.preventDefault();
        ...
        <ListCategory
          ...
          setCategory={(cate) => {
            if (cate.value === "All") {
              ...
              category === ""
                ? dispatch(setfilter({ key: "category", value: cate.value }))
                : dispatch(
                    setfilter({
                      key: "category",
                      value: `${category}|${cate.value}`,
                    })
                  );
            }
          }}
